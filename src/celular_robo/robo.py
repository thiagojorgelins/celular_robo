...
class QuantidadeValida:
    ...
    def __set__(self, instance, valor):
        if valor < 0:
            raise ValueError(f"quantidade coletada não pode ser negativa: {valor}")
        maximo = getattr(instance, "quantidade_pedida", None)
        if maximo is not None and valor > maximo:
            raise ValueError(
                f"quantidade coletada ({valor}) excede a quantidade pedida ({maximo})"
            )
...
class Bandeja:
    ...
    def coletar(self, codinome, quantidade):
        item = self._itens.setdefault(codinome, ItemBandeja(codinome, quantidade))
    ...
    def esta_completa(self):
        return bool(self._itens) and all(item.completo for item in self._itens.values())
...
class RoboColetor(Robo, categoria="coleta"):
    ...
    def __repr__(self):
        return (
            ...
            f"bandeja={len(self.bandeja)} itens)"
        )
    ...
    def processar_pedido(self, pedido):
        ...
        while self._comandos_pendentes and isinstance(self.modo, ModoColetando):
